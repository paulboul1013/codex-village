...
func TestReduceRolloutActivityMapsObserverP0SignalsWithoutLeakingPayloads(t *testing.T) {
	tests := []struct {
		...
		{"real exec approval", `{"type":"event_msg","payload":{"type":"exec_approval_request","command":"private command"}}`, "waiting", "tool", "waiting_for_approval"},
		{"real patch approval", `{"type":"event_msg","payload":{"type":"apply_patch_approval_request","changes":"private patch"}}`, "waiting", "tool", "waiting_for_approval"},
		{"real permissions approval", `{"type":"event_msg","payload":{"type":"request_permissions","reason":"private reason"}}`, "waiting", "tool", "waiting_for_approval"},
		{"explicit failure", `{"type":"event_msg","payload":{"type":"turn_failed","error":"private error"}}`, "failed", "unknown", "none"},
		{"real explicit error", `{"type":"event_msg","payload":{"type":"error","message":"private error","codex_error_info":"other"}}`, "failed", "unknown", "none"},
